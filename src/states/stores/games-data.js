...
function useGamesData() {
        ...
        onGet: async(key) => {
            try {
                if (window.navigator.onLine && zipcode) {
                    const remoteGames = await fetchApi({url : `${apiBaseUrl}/games/${zipcode}/1/3`, method : 'GET'});
  ...
  const updateGamesData = async (newZipcode, result) => {
    setLoading(true);
    const newGamesData = await fetchApi({url : `${apiBaseUrl}/games/${newZipcode}/1/3`, method : 'GET'});
